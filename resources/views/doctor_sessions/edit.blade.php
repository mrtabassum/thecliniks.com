                ...
                <div class="flex-lg-row-fluid mb-10 mb-lg-0 me-lg-7 me-xl-10">
                    ...
                    <div class="card">
                        <div class="card-body p-12">
                            @if(getLogInUser()->hasRole('doctor'))
                                {{ Form::model($doctorSession,['route' => ['doctors.doctor-sessions.update', $doctorSession->id], 'method' => 'patch','id' => 'saveForm']) }}
                            @else
                                {{ Form::model($doctorSession,['route' => ['doctor-sessions.update', $doctorSession->id], 'method' => 'patch','id' => 'saveForm']) }}
                            @endif
                            <div class="">
                                @if(getLogInUser()->hasRole('doctor'))
...
@section('page_js')
    <script>
        let isEdit = true;
        let sessionTime = "{{ $doctorSession->session_time }}";
        let gapBtwAppointment = "{{ $doctorSession->gap_appointment }}";
        let doctorId = "{{$doctorSession->doctor_id}}";
